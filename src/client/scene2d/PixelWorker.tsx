...
export function PixelWorker({ worker, helperCount = 0, confirmedHelperCount = 0, language = 'en', onSelect }: PixelWorkerProps) {
  ...
  return (
    <button
      type="button"
      className={`pixel-worker pixel-worker--${worker.tool} pixel-worker--${state} pixel-worker--${role}`}
    ...
    >
      ...
      {state === 'waiting' && <em className="pixel-worker__bubble" data-testid="worker-bubble">…</em>}
