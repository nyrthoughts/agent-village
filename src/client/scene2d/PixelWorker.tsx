...
export function PixelWorker({ worker, helperCount = 0, onSelect }: PixelWorkerProps) {
  ...
  return (
    <button
      ...
      aria-label={`${TOOL_LABELS[worker.tool]} ${role} agent, ${worker.state}, ${title}`}
    ...
    >
      ...
      {helperCount > 0 && <span className="pixel-worker__count" aria-label={`${helperCount} helper agents`}>{helperCount}</span>}
    </button>
  );
}
